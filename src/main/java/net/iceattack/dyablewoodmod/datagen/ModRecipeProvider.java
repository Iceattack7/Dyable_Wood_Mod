package net.iceattack.dyablewoodmod.datagen;

import net.iceattack.dyablewoodmod.block.ModBlocks;
import net.iceattack.dyablewoodmod.item.ModItems;
import net.minecraft.core.HolderLookup;
import net.minecraft.data.PackOutput;
import net.minecraft.data.recipes.RecipeCategory;
import net.minecraft.data.recipes.RecipeOutput;
import net.minecraft.data.recipes.RecipeProvider;
import net.minecraft.tags.ItemTags;
import net.minecraft.world.item.Items;
import net.minecraft.world.item.crafting.Ingredient;
import net.minecraft.world.level.block.Blocks;
import net.neoforged.neoforge.common.Tags;

import java.util.concurrent.CompletableFuture;

public class ModRecipeProvider extends RecipeProvider {
    public ModRecipeProvider(HolderLookup.Provider registries, RecipeOutput output) {
        super(registries, output);
    }

    public static class Runner extends RecipeProvider.Runner {


        public Runner(PackOutput packOutput, CompletableFuture<HolderLookup.Provider> registries) {
            super(packOutput, registries);
        }

        @Override
        protected RecipeProvider createRecipeProvider(HolderLookup.Provider registries, RecipeOutput output) {
            return new ModRecipeProvider(registries, output);
        }

        @Override
        public String getName() {
            return "Dyable Wood Mod Recipes";
        }
    }

    @Override
    protected void buildRecipes() {

        shapeless(RecipeCategory.MISC, ModItems.LIGHT_PURPLE_DYE.get(), 9)
                .requires(Tags.Items.DYES_PURPLE)
                .requires(Tags.Items.DYES_WHITE)
                .unlockedBy(getHasName(Items.RED_TULIP), has(Items.RED_TULIP))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_PURPLE_PLANKS, 1)
                .requires(ModItems.LIGHT_PURPLE_DYE)
                .requires(ItemTags.PLANKS)
                .unlockedBy(getHasName(ModItems.LIGHT_PURPLE_DYE), has(ModItems.LIGHT_PURPLE_DYE))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BLACK_PLANKS, 1)
                .requires(Tags.Items.DYES_BLACK)
                .requires(ItemTags.PLANKS)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BLUE_PLANKS, 1)
                .requires(Tags.Items.DYES_BLUE)
                .requires(ItemTags.PLANKS)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BROWN_PLANKS, 1)
                .requires(Tags.Items.DYES_BROWN)
                .requires(ItemTags.PLANKS)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.CYAN_PLANKS, 1)
                .requires(Tags.Items.DYES_CYAN)
                .requires(ItemTags.PLANKS)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.GRAY_PLANKS, 1)
                .requires(Tags.Items.DYES_GRAY)
                .requires(ItemTags.PLANKS)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.GREEN_PLANKS, 1)
                .requires(Tags.Items.DYES_GREEN)
                .requires(ItemTags.PLANKS)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_BLUE_PLANKS, 1)
                .requires(Tags.Items.DYES_LIGHT_BLUE)
                .requires(ItemTags.PLANKS)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_GRAY_PLANKS, 1)
                .requires(Tags.Items.DYES_LIGHT_GRAY)
                .requires(ItemTags.PLANKS)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIME_PLANKS, 1)
                .requires(Tags.Items.DYES_LIME)
                .requires(ItemTags.PLANKS)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.MAGENTA_PLANKS, 1)
                .requires(Tags.Items.DYES_MAGENTA)
                .requires(ItemTags.PLANKS)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.ORANGE_PLANKS, 1)
                .requires(Tags.Items.DYES_ORANGE)
                .requires(ItemTags.PLANKS)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.PINK_PLANKS, 1)
                .requires(Tags.Items.DYES_PINK)
                .requires(ItemTags.PLANKS)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.PURPLE_PLANKS, 1)
                .requires(Tags.Items.DYES_PURPLE)
                .requires(ItemTags.PLANKS)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.RED_PLANKS, 1)
                .requires(Tags.Items.DYES_RED)
                .requires(ItemTags.PLANKS)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.WHITE_PLANKS, 1)
                .requires(Tags.Items.DYES_WHITE)
                .requires(ItemTags.PLANKS)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.YELLOW_PLANKS, 1)
                .requires(Tags.Items.DYES_YELLOW)
                .requires(ItemTags.PLANKS)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        stairBuilder(ModBlocks.BLUE_WOOD_STAIRS.get(), Ingredient.of(ModBlocks.BLUE_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        slab(RecipeCategory.BUILDING_BLOCKS, ModBlocks.BLUE_WOOD_SLAB, ModBlocks.BLUE_PLANKS);

        buttonBuilder(ModBlocks.BLUE_WOOD_BUTTON, Ingredient.of(ModBlocks.BLUE_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);
        pressurePlate(ModBlocks.BLUE_WOOD_PRESSURE_PLATE, ModBlocks.BLUE_PLANKS);

        fenceBuilder(ModBlocks.BLUE_WOOD_FENCE, Ingredient.of(ModBlocks.BLUE_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        fenceGateBuilder(ModBlocks.BLUE_WOOD_FENCE_GATE, Ingredient.of(ModBlocks.BLUE_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BLUE_ACACIA_DOOR, 1)
                .requires(Tags.Items.DYES_BLUE)
                .requires(Blocks.ACACIA_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BLUE_ACACIA_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_BLUE)
                .requires(Blocks.ACACIA_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BLUE_BIRCH_DOOR, 1)
                .requires(Tags.Items.DYES_BLUE)
                .requires(Blocks.BIRCH_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BLUE_BIRCH_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_BLUE)
                .requires(Blocks.BIRCH_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BLUE_DARK_OAK_DOOR, 1)
                .requires(Tags.Items.DYES_BLUE)
                .requires(Blocks.DARK_OAK_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BLUE_DARK_OAK_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_BLUE)
                .requires(Blocks.DARK_OAK_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BLUE_JUNGLE_DOOR, 1)
                .requires(Tags.Items.DYES_BLUE)
                .requires(Blocks.JUNGLE_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BLUE_JUNGLE_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_BLUE)
                .requires(Blocks.JUNGLE_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BLUE_OAK_DOOR, 1)
                .requires(Tags.Items.DYES_BLUE)
                .requires(Blocks.OAK_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BLUE_OAK_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_BLUE)
                .requires(Blocks.OAK_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BLUE_SPRUCE_DOOR, 1)
                .requires(Tags.Items.DYES_BLUE)
                .requires(Blocks.SPRUCE_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BLUE_SPRUCE_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_BLUE)
                .requires(Blocks.SPRUCE_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        stairBuilder(ModBlocks.BLACK_WOOD_STAIRS, Ingredient.of(ModBlocks.BLACK_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        slab(RecipeCategory.BUILDING_BLOCKS, ModBlocks.BLACK_WOOD_SLAB, ModBlocks.BLACK_PLANKS);

        buttonBuilder(ModBlocks.BLACK_WOOD_BUTTON, Ingredient.of(ModBlocks.BLACK_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);
        pressurePlate(ModBlocks.BLACK_WOOD_PRESSURE_PLATE, ModBlocks.BLACK_PLANKS);

        fenceBuilder(ModBlocks.BLACK_WOOD_FENCE, Ingredient.of(ModBlocks.BLACK_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        fenceGateBuilder(ModBlocks.BLACK_WOOD_FENCE_GATE, Ingredient.of(ModBlocks.BLACK_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BLACK_ACACIA_DOOR, 1)
                .requires(Tags.Items.DYES_BLACK)
                .requires(Blocks.ACACIA_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BLACK_ACACIA_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_BLACK)
                .requires(Blocks.ACACIA_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BLACK_BIRCH_DOOR, 1)
                .requires(Tags.Items.DYES_BLACK)
                .requires(Blocks.BIRCH_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BLACK_BIRCH_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_BLACK)
                .requires(Blocks.BIRCH_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BLACK_DARK_OAK_DOOR, 1)
                .requires(Tags.Items.DYES_BLACK)
                .requires(Blocks.DARK_OAK_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BLACK_DARK_OAK_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_BLACK)
                .requires(Blocks.DARK_OAK_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BLACK_JUNGLE_DOOR, 1)
                .requires(Tags.Items.DYES_BLACK)
                .requires(Blocks.JUNGLE_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BLACK_JUNGLE_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_BLACK)
                .requires(Blocks.JUNGLE_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BLACK_OAK_DOOR, 1)
                .requires(Tags.Items.DYES_BLACK)
                .requires(Blocks.OAK_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BLACK_OAK_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_BLACK)
                .requires(Blocks.OAK_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BLACK_SPRUCE_DOOR, 1)
                .requires(Tags.Items.DYES_BLACK)
                .requires(Blocks.SPRUCE_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BLACK_SPRUCE_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_BLACK)
                .requires(Blocks.SPRUCE_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        stairBuilder(ModBlocks.BROWN_WOOD_STAIRS, Ingredient.of(ModBlocks.BROWN_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        slab(RecipeCategory.BUILDING_BLOCKS, ModBlocks.BROWN_WOOD_SLAB, ModBlocks.BROWN_PLANKS);

        buttonBuilder(ModBlocks.BROWN_WOOD_BUTTON, Ingredient.of(ModBlocks.BROWN_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);
        pressurePlate(ModBlocks.BROWN_WOOD_PRESSURE_PLATE, ModBlocks.BROWN_PLANKS);

        fenceBuilder(ModBlocks.BROWN_WOOD_FENCE, Ingredient.of(ModBlocks.BROWN_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        fenceGateBuilder(ModBlocks.BROWN_WOOD_FENCE_GATE, Ingredient.of(ModBlocks.BROWN_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BROWN_ACACIA_DOOR, 1)
                .requires(Tags.Items.DYES_BROWN)
                .requires(Blocks.ACACIA_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BROWN_ACACIA_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_BROWN)
                .requires(Blocks.ACACIA_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BROWN_BIRCH_DOOR, 1)
                .requires(Tags.Items.DYES_BROWN)
                .requires(Blocks.BIRCH_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BROWN_BIRCH_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_BROWN)
                .requires(Blocks.BIRCH_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BROWN_DARK_OAK_DOOR, 1)
                .requires(Tags.Items.DYES_BROWN)
                .requires(Blocks.DARK_OAK_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BROWN_DARK_OAK_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_BROWN)
                .requires(Blocks.DARK_OAK_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BROWN_JUNGLE_DOOR, 1)
                .requires(Tags.Items.DYES_BROWN)
                .requires(Blocks.JUNGLE_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BROWN_JUNGLE_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_BROWN)
                .requires(Blocks.JUNGLE_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BROWN_OAK_DOOR, 1)
                .requires(Tags.Items.DYES_BROWN)
                .requires(Blocks.OAK_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BROWN_OAK_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_BROWN)
                .requires(Blocks.OAK_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BROWN_SPRUCE_DOOR, 1)
                .requires(Tags.Items.DYES_BROWN)
                .requires(Blocks.SPRUCE_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.BROWN_SPRUCE_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_BROWN)
                .requires(Blocks.SPRUCE_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        stairBuilder(ModBlocks.CYAN_WOOD_STAIRS, Ingredient.of(ModBlocks.CYAN_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        slab(RecipeCategory.BUILDING_BLOCKS, ModBlocks.CYAN_WOOD_SLAB, ModBlocks.CYAN_PLANKS);

        buttonBuilder(ModBlocks.CYAN_WOOD_BUTTON, Ingredient.of(ModBlocks.CYAN_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);
        pressurePlate(ModBlocks.CYAN_WOOD_PRESSURE_PLATE, ModBlocks.CYAN_PLANKS);

        fenceBuilder(ModBlocks.CYAN_WOOD_FENCE, Ingredient.of(ModBlocks.CYAN_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        fenceGateBuilder(ModBlocks.CYAN_WOOD_FENCE_GATE, Ingredient.of(ModBlocks.CYAN_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.CYAN_ACACIA_DOOR, 1)
                .requires(Tags.Items.DYES_CYAN)
                .requires(Blocks.ACACIA_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.CYAN_ACACIA_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_CYAN)
                .requires(Blocks.ACACIA_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.CYAN_BIRCH_DOOR, 1)
                .requires(Tags.Items.DYES_CYAN)
                .requires(Blocks.BIRCH_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.CYAN_BIRCH_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_CYAN)
                .requires(Blocks.BIRCH_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.CYAN_DARK_OAK_DOOR, 1)
                .requires(Tags.Items.DYES_CYAN)
                .requires(Blocks.DARK_OAK_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.CYAN_DARK_OAK_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_CYAN)
                .requires(Blocks.DARK_OAK_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.CYAN_JUNGLE_DOOR, 1)
                .requires(Tags.Items.DYES_CYAN)
                .requires(Blocks.JUNGLE_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.CYAN_JUNGLE_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_CYAN)
                .requires(Blocks.JUNGLE_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.CYAN_OAK_DOOR, 1)
                .requires(Tags.Items.DYES_CYAN)
                .requires(Blocks.OAK_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.CYAN_OAK_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_CYAN)
                .requires(Blocks.OAK_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.CYAN_SPRUCE_DOOR, 1)
                .requires(Tags.Items.DYES_CYAN)
                .requires(Blocks.SPRUCE_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.CYAN_SPRUCE_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_CYAN)
                .requires(Blocks.SPRUCE_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        stairBuilder(ModBlocks.GRAY_WOOD_STAIRS, Ingredient.of(ModBlocks.GRAY_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        slab(RecipeCategory.BUILDING_BLOCKS, ModBlocks.GRAY_WOOD_SLAB, ModBlocks.GRAY_PLANKS);

        buttonBuilder(ModBlocks.GRAY_WOOD_BUTTON, Ingredient.of(ModBlocks.GRAY_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);
        pressurePlate(ModBlocks.GRAY_WOOD_PRESSURE_PLATE, ModBlocks.GRAY_PLANKS);

        fenceBuilder(ModBlocks.GRAY_WOOD_FENCE, Ingredient.of(ModBlocks.GRAY_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        fenceGateBuilder(ModBlocks.GRAY_WOOD_FENCE_GATE, Ingredient.of(ModBlocks.GRAY_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.GRAY_ACACIA_DOOR, 1)
                .requires(Tags.Items.DYES_GRAY)
                .requires(Blocks.ACACIA_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.GRAY_ACACIA_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_GRAY)
                .requires(Blocks.ACACIA_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.GRAY_BIRCH_DOOR, 1)
                .requires(Tags.Items.DYES_GRAY)
                .requires(Blocks.BIRCH_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.GRAY_BIRCH_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_GRAY)
                .requires(Blocks.BIRCH_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.GRAY_DARK_OAK_DOOR, 1)
                .requires(Tags.Items.DYES_GRAY)
                .requires(Blocks.DARK_OAK_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.GRAY_DARK_OAK_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_GRAY)
                .requires(Blocks.DARK_OAK_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.GRAY_JUNGLE_DOOR, 1)
                .requires(Tags.Items.DYES_GRAY)
                .requires(Blocks.JUNGLE_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.GRAY_JUNGLE_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_GRAY)
                .requires(Blocks.JUNGLE_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.GRAY_OAK_DOOR, 1)
                .requires(Tags.Items.DYES_GRAY)
                .requires(Blocks.OAK_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.GRAY_OAK_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_GRAY)
                .requires(Blocks.OAK_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.GRAY_SPRUCE_DOOR, 1)
                .requires(Tags.Items.DYES_GRAY)
                .requires(Blocks.SPRUCE_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.GRAY_SPRUCE_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_GRAY)
                .requires(Blocks.SPRUCE_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        stairBuilder(ModBlocks.GREEN_WOOD_STAIRS, Ingredient.of(ModBlocks.GREEN_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        slab(RecipeCategory.BUILDING_BLOCKS, ModBlocks.GREEN_WOOD_SLAB, ModBlocks.GREEN_PLANKS);

        buttonBuilder(ModBlocks.GREEN_WOOD_BUTTON, Ingredient.of(ModBlocks.GREEN_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);
        pressurePlate(ModBlocks.GREEN_WOOD_PRESSURE_PLATE, ModBlocks.GREEN_PLANKS);

        fenceBuilder(ModBlocks.GREEN_WOOD_FENCE, Ingredient.of(ModBlocks.GREEN_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        fenceGateBuilder(ModBlocks.GREEN_WOOD_FENCE_GATE, Ingredient.of(ModBlocks.GREEN_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.GREEN_ACACIA_DOOR, 1)
                .requires(Tags.Items.DYES_GREEN)
                .requires(Blocks.ACACIA_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.GREEN_ACACIA_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_GREEN)
                .requires(Blocks.ACACIA_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.GREEN_BIRCH_DOOR, 1)
                .requires(Tags.Items.DYES_GREEN)
                .requires(Blocks.BIRCH_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.GREEN_BIRCH_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_GREEN)
                .requires(Blocks.BIRCH_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.GREEN_DARK_OAK_DOOR, 1)
                .requires(Tags.Items.DYES_GREEN)
                .requires(Blocks.DARK_OAK_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.GREEN_DARK_OAK_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_GREEN)
                .requires(Blocks.DARK_OAK_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.GREEN_JUNGLE_DOOR, 1)
                .requires(Tags.Items.DYES_GREEN)
                .requires(Blocks.JUNGLE_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.GREEN_JUNGLE_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_GREEN)
                .requires(Blocks.JUNGLE_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.GREEN_OAK_DOOR, 1)
                .requires(Tags.Items.DYES_GREEN)
                .requires(Blocks.OAK_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.GREEN_OAK_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_GREEN)
                .requires(Blocks.OAK_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.GREEN_SPRUCE_DOOR, 1)
                .requires(Tags.Items.DYES_GREEN)
                .requires(Blocks.SPRUCE_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.GREEN_SPRUCE_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_GREEN)
                .requires(Blocks.SPRUCE_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        stairBuilder(ModBlocks.LIGHT_BLUE_WOOD_STAIRS, Ingredient.of(ModBlocks.LIGHT_BLUE_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        slab(RecipeCategory.BUILDING_BLOCKS, ModBlocks.LIGHT_BLUE_WOOD_SLAB, ModBlocks.LIGHT_BLUE_PLANKS);

        buttonBuilder(ModBlocks.LIGHT_BLUE_WOOD_BUTTON, Ingredient.of(ModBlocks.LIGHT_BLUE_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);
        pressurePlate(ModBlocks.LIGHT_BLUE_WOOD_PRESSURE_PLATE, ModBlocks.LIGHT_BLUE_PLANKS);

        fenceBuilder(ModBlocks.LIGHT_BLUE_WOOD_FENCE, Ingredient.of(ModBlocks.LIGHT_BLUE_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        fenceGateBuilder(ModBlocks.LIGHT_BLUE_WOOD_FENCE_GATE, Ingredient.of(ModBlocks.LIGHT_BLUE_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_BLUE_ACACIA_DOOR, 1)
                .requires(Tags.Items.DYES_LIGHT_BLUE)
                .requires(Blocks.ACACIA_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_BLUE_ACACIA_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_LIGHT_BLUE)
                .requires(Blocks.ACACIA_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_BLUE_BIRCH_DOOR, 1)
                .requires(Tags.Items.DYES_LIGHT_BLUE)
                .requires(Blocks.BIRCH_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_BLUE_BIRCH_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_LIGHT_BLUE)
                .requires(Blocks.BIRCH_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_BLUE_DARK_OAK_DOOR, 1)
                .requires(Tags.Items.DYES_LIGHT_BLUE)
                .requires(Blocks.DARK_OAK_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_BLUE_DARK_OAK_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_LIGHT_BLUE)
                .requires(Blocks.DARK_OAK_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_BLUE_JUNGLE_DOOR, 1)
                .requires(Tags.Items.DYES_LIGHT_BLUE)
                .requires(Blocks.JUNGLE_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_BLUE_JUNGLE_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_LIGHT_BLUE)
                .requires(Blocks.JUNGLE_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_BLUE_OAK_DOOR, 1)
                .requires(Tags.Items.DYES_LIGHT_BLUE)
                .requires(Blocks.OAK_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_BLUE_OAK_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_LIGHT_BLUE)
                .requires(Blocks.OAK_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_BLUE_SPRUCE_DOOR, 1)
                .requires(Tags.Items.DYES_LIGHT_BLUE)
                .requires(Blocks.SPRUCE_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_BLUE_SPRUCE_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_LIGHT_BLUE)
                .requires(Blocks.SPRUCE_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        stairBuilder(ModBlocks.LIGHT_GRAY_WOOD_STAIRS, Ingredient.of(ModBlocks.LIGHT_GRAY_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        slab(RecipeCategory.BUILDING_BLOCKS, ModBlocks.LIGHT_GRAY_WOOD_SLAB, ModBlocks.LIGHT_GRAY_PLANKS);

        buttonBuilder(ModBlocks.LIGHT_GRAY_WOOD_BUTTON, Ingredient.of(ModBlocks.LIGHT_GRAY_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);
        pressurePlate(ModBlocks.LIGHT_GRAY_WOOD_PRESSURE_PLATE, ModBlocks.LIGHT_GRAY_PLANKS);

        fenceBuilder(ModBlocks.LIGHT_GRAY_WOOD_FENCE, Ingredient.of(ModBlocks.LIGHT_GRAY_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        fenceGateBuilder(ModBlocks.LIGHT_GRAY_WOOD_FENCE_GATE, Ingredient.of(ModBlocks.LIGHT_GRAY_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_GRAY_ACACIA_DOOR, 1)
                .requires(Tags.Items.DYES_LIGHT_GRAY)
                .requires(Blocks.ACACIA_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_GRAY_ACACIA_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_LIGHT_GRAY)
                .requires(Blocks.ACACIA_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_GRAY_BIRCH_DOOR, 1)
                .requires(Tags.Items.DYES_LIGHT_GRAY)
                .requires(Blocks.BIRCH_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_GRAY_BIRCH_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_LIGHT_GRAY)
                .requires(Blocks.BIRCH_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_GRAY_DARK_OAK_DOOR, 1)
                .requires(Tags.Items.DYES_LIGHT_GRAY)
                .requires(Blocks.DARK_OAK_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_GRAY_DARK_OAK_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_LIGHT_GRAY)
                .requires(Blocks.DARK_OAK_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_GRAY_JUNGLE_DOOR, 1)
                .requires(Tags.Items.DYES_LIGHT_GRAY)
                .requires(Blocks.JUNGLE_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_GRAY_JUNGLE_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_LIGHT_GRAY)
                .requires(Blocks.JUNGLE_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_GRAY_OAK_DOOR, 1)
                .requires(Tags.Items.DYES_LIGHT_GRAY)
                .requires(Blocks.OAK_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_GRAY_OAK_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_LIGHT_GRAY)
                .requires(Blocks.OAK_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_GRAY_SPRUCE_DOOR, 1)
                .requires(Tags.Items.DYES_LIGHT_GRAY)
                .requires(Blocks.SPRUCE_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_GRAY_SPRUCE_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_LIGHT_GRAY)
                .requires(Blocks.SPRUCE_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        stairBuilder(ModBlocks.LIGHT_PURPLE_WOOD_STAIRS, Ingredient.of(ModBlocks.LIGHT_PURPLE_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        slab(RecipeCategory.BUILDING_BLOCKS, ModBlocks.LIGHT_PURPLE_WOOD_SLAB, ModBlocks.LIGHT_PURPLE_PLANKS);

        buttonBuilder(ModBlocks.LIGHT_PURPLE_WOOD_BUTTON, Ingredient.of(ModBlocks.LIGHT_PURPLE_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);
        pressurePlate(ModBlocks.LIGHT_PURPLE_WOOD_PRESSURE_PLATE, ModBlocks.LIGHT_PURPLE_PLANKS);

        fenceBuilder(ModBlocks.LIGHT_PURPLE_WOOD_FENCE, Ingredient.of(ModBlocks.LIGHT_PURPLE_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        fenceGateBuilder(ModBlocks.LIGHT_PURPLE_WOOD_FENCE_GATE, Ingredient.of(ModBlocks.LIGHT_PURPLE_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_PURPLE_ACACIA_DOOR, 1)
                .requires(ModItems.LIGHT_PURPLE_DYE)
                .requires(Blocks.ACACIA_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_PURPLE_ACACIA_TRAPDOOR, 1)
                .requires(ModItems.LIGHT_PURPLE_DYE)
                .requires(Blocks.ACACIA_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_PURPLE_BIRCH_DOOR, 1)
                .requires(ModItems.LIGHT_PURPLE_DYE)
                .requires(Blocks.BIRCH_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_PURPLE_BIRCH_TRAPDOOR, 1)
                .requires(ModItems.LIGHT_PURPLE_DYE)
                .requires(Blocks.BIRCH_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_PURPLE_DARK_OAK_DOOR, 1)
                .requires(ModItems.LIGHT_PURPLE_DYE)
                .requires(Blocks.DARK_OAK_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_PURPLE_DARK_OAK_TRAPDOOR, 1)
                .requires(ModItems.LIGHT_PURPLE_DYE)
                .requires(Blocks.DARK_OAK_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_PURPLE_JUNGLE_DOOR, 1)
                .requires(ModItems.LIGHT_PURPLE_DYE)
                .requires(Blocks.JUNGLE_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_PURPLE_JUNGLE_TRAPDOOR, 1)
                .requires(ModItems.LIGHT_PURPLE_DYE)
                .requires(Blocks.JUNGLE_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_PURPLE_OAK_DOOR, 1)
                .requires(ModItems.LIGHT_PURPLE_DYE)
                .requires(Blocks.OAK_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_PURPLE_OAK_TRAPDOOR, 1)
                .requires(ModItems.LIGHT_PURPLE_DYE)
                .requires(Blocks.OAK_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_PURPLE_SPRUCE_DOOR, 1)
                .requires(ModItems.LIGHT_PURPLE_DYE)
                .requires(Blocks.SPRUCE_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIGHT_PURPLE_SPRUCE_TRAPDOOR, 1)
                .requires(ModItems.LIGHT_PURPLE_DYE)
                .requires(Blocks.SPRUCE_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        stairBuilder(ModBlocks.LIME_WOOD_STAIRS, Ingredient.of(ModBlocks.LIME_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        slab(RecipeCategory.BUILDING_BLOCKS, ModBlocks.LIME_WOOD_SLAB, ModBlocks.LIME_PLANKS);

        buttonBuilder(ModBlocks.LIME_WOOD_BUTTON, Ingredient.of(ModBlocks.LIME_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);
        pressurePlate(ModBlocks.LIME_WOOD_PRESSURE_PLATE, ModBlocks.LIME_PLANKS);

        fenceBuilder(ModBlocks.LIME_WOOD_FENCE, Ingredient.of(ModBlocks.LIME_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        fenceGateBuilder(ModBlocks.LIME_WOOD_FENCE_GATE, Ingredient.of(ModBlocks.LIME_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIME_ACACIA_DOOR, 1)
                .requires(Tags.Items.DYES_LIME)
                .requires(Blocks.ACACIA_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIME_ACACIA_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_LIME)
                .requires(Blocks.ACACIA_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIME_BIRCH_DOOR, 1)
                .requires(Tags.Items.DYES_LIME)
                .requires(Blocks.BIRCH_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIME_BIRCH_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_LIME)
                .requires(Blocks.BIRCH_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIME_DARK_OAK_DOOR, 1)
                .requires(Tags.Items.DYES_LIME)
                .requires(Blocks.DARK_OAK_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIME_DARK_OAK_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_LIME)
                .requires(Blocks.DARK_OAK_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIME_JUNGLE_DOOR, 1)
                .requires(Tags.Items.DYES_LIME)
                .requires(Blocks.JUNGLE_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIME_JUNGLE_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_LIME)
                .requires(Blocks.JUNGLE_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIME_OAK_DOOR, 1)
                .requires(Tags.Items.DYES_LIME)
                .requires(Blocks.OAK_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIME_OAK_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_LIME)
                .requires(Blocks.OAK_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIME_SPRUCE_DOOR, 1)
                .requires(Tags.Items.DYES_LIME)
                .requires(Blocks.SPRUCE_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.LIME_SPRUCE_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_LIME)
                .requires(Blocks.SPRUCE_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        stairBuilder(ModBlocks.MAGENTA_WOOD_STAIRS, Ingredient.of(ModBlocks.MAGENTA_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        slab(RecipeCategory.BUILDING_BLOCKS, ModBlocks.MAGENTA_WOOD_SLAB, ModBlocks.MAGENTA_PLANKS);

        buttonBuilder(ModBlocks.MAGENTA_WOOD_BUTTON, Ingredient.of(ModBlocks.MAGENTA_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);
        pressurePlate(ModBlocks.MAGENTA_WOOD_PRESSURE_PLATE, ModBlocks.MAGENTA_PLANKS);

        fenceBuilder(ModBlocks.MAGENTA_WOOD_FENCE, Ingredient.of(ModBlocks.MAGENTA_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        fenceGateBuilder(ModBlocks.MAGENTA_WOOD_FENCE_GATE, Ingredient.of(ModBlocks.MAGENTA_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.MAGENTA_ACACIA_DOOR, 1)
                .requires(Tags.Items.DYES_MAGENTA)
                .requires(Blocks.ACACIA_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.MAGENTA_ACACIA_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_MAGENTA)
                .requires(Blocks.ACACIA_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.MAGENTA_BIRCH_DOOR, 1)
                .requires(Tags.Items.DYES_MAGENTA)
                .requires(Blocks.BIRCH_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.MAGENTA_BIRCH_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_MAGENTA)
                .requires(Blocks.BIRCH_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.MAGENTA_DARK_OAK_DOOR, 1)
                .requires(Tags.Items.DYES_MAGENTA)
                .requires(Blocks.DARK_OAK_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.MAGENTA_DARK_OAK_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_MAGENTA)
                .requires(Blocks.DARK_OAK_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.MAGENTA_JUNGLE_DOOR, 1)
                .requires(Tags.Items.DYES_MAGENTA)
                .requires(Blocks.JUNGLE_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.MAGENTA_JUNGLE_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_MAGENTA)
                .requires(Blocks.JUNGLE_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.MAGENTA_OAK_DOOR, 1)
                .requires(Tags.Items.DYES_MAGENTA)
                .requires(Blocks.OAK_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.MAGENTA_OAK_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_MAGENTA)
                .requires(Blocks.OAK_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.MAGENTA_SPRUCE_DOOR, 1)
                .requires(Tags.Items.DYES_MAGENTA)
                .requires(Blocks.SPRUCE_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.MAGENTA_SPRUCE_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_MAGENTA)
                .requires(Blocks.SPRUCE_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        stairBuilder(ModBlocks.ORANGE_WOOD_STAIRS, Ingredient.of(ModBlocks.ORANGE_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        slab(RecipeCategory.BUILDING_BLOCKS, ModBlocks.ORANGE_WOOD_SLAB, ModBlocks.ORANGE_PLANKS);

        buttonBuilder(ModBlocks.ORANGE_WOOD_BUTTON, Ingredient.of(ModBlocks.ORANGE_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);
        pressurePlate(ModBlocks.ORANGE_WOOD_PRESSURE_PLATE, ModBlocks.ORANGE_PLANKS);

        fenceBuilder(ModBlocks.ORANGE_WOOD_FENCE, Ingredient.of(ModBlocks.ORANGE_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        fenceGateBuilder(ModBlocks.ORANGE_WOOD_FENCE_GATE, Ingredient.of(ModBlocks.ORANGE_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.ORANGE_ACACIA_DOOR, 1)
                .requires(Tags.Items.DYES_ORANGE)
                .requires(Blocks.ACACIA_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.ORANGE_ACACIA_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_ORANGE)
                .requires(Blocks.ACACIA_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.ORANGE_BIRCH_DOOR, 1)
                .requires(Tags.Items.DYES_ORANGE)
                .requires(Blocks.BIRCH_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.ORANGE_BIRCH_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_ORANGE)
                .requires(Blocks.BIRCH_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.ORANGE_DARK_OAK_DOOR, 1)
                .requires(Tags.Items.DYES_ORANGE)
                .requires(Blocks.DARK_OAK_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.ORANGE_DARK_OAK_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_ORANGE)
                .requires(Blocks.DARK_OAK_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.ORANGE_JUNGLE_DOOR, 1)
                .requires(Tags.Items.DYES_ORANGE)
                .requires(Blocks.JUNGLE_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.ORANGE_JUNGLE_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_ORANGE)
                .requires(Blocks.JUNGLE_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.ORANGE_OAK_DOOR, 1)
                .requires(Tags.Items.DYES_ORANGE)
                .requires(Blocks.OAK_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.ORANGE_OAK_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_ORANGE)
                .requires(Blocks.OAK_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.ORANGE_SPRUCE_DOOR, 1)
                .requires(Tags.Items.DYES_ORANGE)
                .requires(Blocks.SPRUCE_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.ORANGE_SPRUCE_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_ORANGE)
                .requires(Blocks.SPRUCE_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        stairBuilder(ModBlocks.PINK_WOOD_STAIRS, Ingredient.of(ModBlocks.PINK_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        slab(RecipeCategory.BUILDING_BLOCKS, ModBlocks.PINK_WOOD_SLAB, ModBlocks.PINK_PLANKS);

        buttonBuilder(ModBlocks.PINK_WOOD_BUTTON, Ingredient.of(ModBlocks.PINK_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);
        pressurePlate(ModBlocks.PINK_WOOD_PRESSURE_PLATE, ModBlocks.PINK_PLANKS);

        fenceBuilder(ModBlocks.PINK_WOOD_FENCE, Ingredient.of(ModBlocks.PINK_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        fenceGateBuilder(ModBlocks.PINK_WOOD_FENCE_GATE, Ingredient.of(ModBlocks.PINK_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.PINK_ACACIA_DOOR, 1)
                .requires(Tags.Items.DYES_PINK)
                .requires(Blocks.ACACIA_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.PINK_ACACIA_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_PINK)
                .requires(Blocks.ACACIA_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.PINK_BIRCH_DOOR, 1)
                .requires(Tags.Items.DYES_PINK)
                .requires(Blocks.BIRCH_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.PINK_BIRCH_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_PINK)
                .requires(Blocks.BIRCH_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.PINK_DARK_OAK_DOOR, 1)
                .requires(Tags.Items.DYES_PINK)
                .requires(Blocks.DARK_OAK_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.PINK_DARK_OAK_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_PINK)
                .requires(Blocks.DARK_OAK_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.PINK_JUNGLE_DOOR, 1)
                .requires(Tags.Items.DYES_PINK)
                .requires(Blocks.JUNGLE_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.PINK_JUNGLE_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_PINK)
                .requires(Blocks.JUNGLE_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.PINK_OAK_DOOR, 1)
                .requires(Tags.Items.DYES_PINK)
                .requires(Blocks.OAK_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.PINK_OAK_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_PINK)
                .requires(Blocks.OAK_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.PINK_SPRUCE_DOOR, 1)
                .requires(Tags.Items.DYES_PINK)
                .requires(Blocks.SPRUCE_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.PINK_SPRUCE_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_PINK)
                .requires(Blocks.SPRUCE_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        stairBuilder(ModBlocks.PURPLE_WOOD_STAIRS, Ingredient.of(ModBlocks.PURPLE_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        slab(RecipeCategory.BUILDING_BLOCKS, ModBlocks.PURPLE_WOOD_SLAB, ModBlocks.PURPLE_PLANKS);

        buttonBuilder(ModBlocks.PURPLE_WOOD_BUTTON, Ingredient.of(ModBlocks.PURPLE_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);
        pressurePlate(ModBlocks.PURPLE_WOOD_PRESSURE_PLATE, ModBlocks.PURPLE_PLANKS);

        fenceBuilder(ModBlocks.PURPLE_WOOD_FENCE, Ingredient.of(ModBlocks.PURPLE_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        fenceGateBuilder(ModBlocks.PURPLE_WOOD_FENCE_GATE, Ingredient.of(ModBlocks.PURPLE_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.PURPLE_ACACIA_DOOR, 1)
                .requires(Tags.Items.DYES_PURPLE)
                .requires(Blocks.ACACIA_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.PURPLE_ACACIA_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_PURPLE)
                .requires(Blocks.ACACIA_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.PURPLE_BIRCH_DOOR, 1)
                .requires(Tags.Items.DYES_PURPLE)
                .requires(Blocks.BIRCH_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.PURPLE_BIRCH_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_PURPLE)
                .requires(Blocks.BIRCH_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.PURPLE_DARK_OAK_DOOR, 1)
                .requires(Tags.Items.DYES_PURPLE)
                .requires(Blocks.DARK_OAK_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.PURPLE_DARK_OAK_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_PURPLE)
                .requires(Blocks.DARK_OAK_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.PURPLE_JUNGLE_DOOR, 1)
                .requires(Tags.Items.DYES_PURPLE)
                .requires(Blocks.JUNGLE_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.PURPLE_JUNGLE_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_PURPLE)
                .requires(Blocks.JUNGLE_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.PURPLE_OAK_DOOR, 1)
                .requires(Tags.Items.DYES_PURPLE)
                .requires(Blocks.OAK_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.PURPLE_OAK_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_PURPLE)
                .requires(Blocks.OAK_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.PURPLE_SPRUCE_DOOR, 1)
                .requires(Tags.Items.DYES_PURPLE)
                .requires(Blocks.SPRUCE_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.PURPLE_SPRUCE_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_PURPLE)
                .requires(Blocks.SPRUCE_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        stairBuilder(ModBlocks.RED_WOOD_STAIRS, Ingredient.of(ModBlocks.RED_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        slab(RecipeCategory.BUILDING_BLOCKS, ModBlocks.RED_WOOD_SLAB, ModBlocks.RED_PLANKS);

        buttonBuilder(ModBlocks.RED_WOOD_BUTTON, Ingredient.of(ModBlocks.RED_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);
        pressurePlate(ModBlocks.RED_WOOD_PRESSURE_PLATE, ModBlocks.RED_PLANKS);

        fenceBuilder(ModBlocks.RED_WOOD_FENCE, Ingredient.of(ModBlocks.RED_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        fenceGateBuilder(ModBlocks.RED_WOOD_FENCE_GATE, Ingredient.of(ModBlocks.RED_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.RED_ACACIA_DOOR, 1)
                .requires(Tags.Items.DYES_RED)
                .requires(Blocks.ACACIA_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.RED_ACACIA_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_RED)
                .requires(Blocks.ACACIA_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.RED_BIRCH_DOOR, 1)
                .requires(Tags.Items.DYES_RED)
                .requires(Blocks.BIRCH_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.RED_BIRCH_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_RED)
                .requires(Blocks.BIRCH_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.RED_DARK_OAK_DOOR, 1)
                .requires(Tags.Items.DYES_RED)
                .requires(Blocks.DARK_OAK_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.RED_DARK_OAK_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_RED)
                .requires(Blocks.DARK_OAK_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.RED_JUNGLE_DOOR, 1)
                .requires(Tags.Items.DYES_RED)
                .requires(Blocks.JUNGLE_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.RED_JUNGLE_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_RED)
                .requires(Blocks.JUNGLE_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.RED_OAK_DOOR, 1)
                .requires(Tags.Items.DYES_RED)
                .requires(Blocks.OAK_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.RED_OAK_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_RED)
                .requires(Blocks.OAK_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.RED_SPRUCE_DOOR, 1)
                .requires(Tags.Items.DYES_RED)
                .requires(Blocks.SPRUCE_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.RED_SPRUCE_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_RED)
                .requires(Blocks.SPRUCE_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        stairBuilder(ModBlocks.WHITE_WOOD_STAIRS, Ingredient.of(ModBlocks.WHITE_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        slab(RecipeCategory.BUILDING_BLOCKS, ModBlocks.WHITE_WOOD_SLAB, ModBlocks.WHITE_PLANKS);

        buttonBuilder(ModBlocks.WHITE_WOOD_BUTTON, Ingredient.of(ModBlocks.WHITE_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);
        pressurePlate(ModBlocks.WHITE_WOOD_PRESSURE_PLATE, ModBlocks.WHITE_PLANKS);

        fenceBuilder(ModBlocks.WHITE_WOOD_FENCE, Ingredient.of(ModBlocks.WHITE_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        fenceGateBuilder(ModBlocks.WHITE_WOOD_FENCE_GATE, Ingredient.of(ModBlocks.WHITE_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.WHITE_ACACIA_DOOR, 1)
                .requires(Tags.Items.DYES_WHITE)
                .requires(Blocks.ACACIA_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.WHITE_ACACIA_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_WHITE)
                .requires(Blocks.ACACIA_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.WHITE_BIRCH_DOOR, 1)
                .requires(Tags.Items.DYES_WHITE)
                .requires(Blocks.BIRCH_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.WHITE_BIRCH_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_WHITE)
                .requires(Blocks.BIRCH_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.WHITE_DARK_OAK_DOOR, 1)
                .requires(Tags.Items.DYES_WHITE)
                .requires(Blocks.DARK_OAK_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.WHITE_DARK_OAK_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_WHITE)
                .requires(Blocks.DARK_OAK_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.WHITE_JUNGLE_DOOR, 1)
                .requires(Tags.Items.DYES_WHITE)
                .requires(Blocks.JUNGLE_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.WHITE_JUNGLE_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_WHITE)
                .requires(Blocks.JUNGLE_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.WHITE_OAK_DOOR, 1)
                .requires(Tags.Items.DYES_WHITE)
                .requires(Blocks.OAK_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.WHITE_OAK_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_WHITE)
                .requires(Blocks.OAK_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.WHITE_SPRUCE_DOOR, 1)
                .requires(Tags.Items.DYES_WHITE)
                .requires(Blocks.SPRUCE_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.WHITE_SPRUCE_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_WHITE)
                .requires(Blocks.SPRUCE_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        stairBuilder(ModBlocks.YELLOW_WOOD_STAIRS, Ingredient.of(ModBlocks.YELLOW_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        slab(RecipeCategory.BUILDING_BLOCKS, ModBlocks.YELLOW_WOOD_SLAB, ModBlocks.YELLOW_PLANKS);

        buttonBuilder(ModBlocks.YELLOW_WOOD_BUTTON, Ingredient.of(ModBlocks.YELLOW_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);
        pressurePlate(ModBlocks.YELLOW_WOOD_PRESSURE_PLATE, ModBlocks.YELLOW_PLANKS);

        fenceBuilder(ModBlocks.YELLOW_WOOD_FENCE, Ingredient.of(ModBlocks.YELLOW_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        fenceGateBuilder(ModBlocks.YELLOW_WOOD_FENCE_GATE, Ingredient.of(ModBlocks.YELLOW_PLANKS))
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.YELLOW_ACACIA_DOOR, 1)
                .requires(Tags.Items.DYES_YELLOW)
                .requires(Blocks.ACACIA_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.YELLOW_ACACIA_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_YELLOW)
                .requires(Blocks.ACACIA_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.YELLOW_BIRCH_DOOR, 1)
                .requires(Tags.Items.DYES_YELLOW)
                .requires(Blocks.BIRCH_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.YELLOW_BIRCH_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_YELLOW)
                .requires(Blocks.BIRCH_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.YELLOW_DARK_OAK_DOOR, 1)
                .requires(Tags.Items.DYES_YELLOW)
                .requires(Blocks.DARK_OAK_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.YELLOW_DARK_OAK_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_YELLOW)
                .requires(Blocks.DARK_OAK_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.YELLOW_JUNGLE_DOOR, 1)
                .requires(Tags.Items.DYES_YELLOW)
                .requires(Blocks.JUNGLE_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.YELLOW_JUNGLE_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_YELLOW)
                .requires(Blocks.JUNGLE_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.YELLOW_OAK_DOOR, 1)
                .requires(Tags.Items.DYES_YELLOW)
                .requires(Blocks.OAK_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.YELLOW_OAK_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_YELLOW)
                .requires(Blocks.OAK_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.YELLOW_SPRUCE_DOOR, 1)
                .requires(Tags.Items.DYES_YELLOW)
                .requires(Blocks.SPRUCE_DOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

        shapeless(RecipeCategory.MISC, ModBlocks.YELLOW_SPRUCE_TRAPDOOR, 1)
                .requires(Tags.Items.DYES_YELLOW)
                .requires(Blocks.SPRUCE_TRAPDOOR)
                .unlockedBy("has_planks", has(ItemTags.PLANKS))
                .save(output);

    }
}
